...
type TokenSupplier interface {
	Token() (securityToken, error)
	RefreshToken() error
}

type genericServiceAccountTokenSupplier struct {
	token     securityToken
	RefreshFn func() (securityToken, error)
}

func (s genericServiceAccountTokenSupplier) Token() (securityToken, error) {
	var err error
...
func (s genericServiceAccountTokenSupplier) RefreshToken() error {

	token, err := s.RefreshFn()
	if err != nil {
		return err
	}
...
func newDefaultTokenSupplier() (*genericServiceAccountTokenSupplier, error) {
	return &genericServiceAccountTokenSupplier{
		RefreshFn: func() (securityToken, error) {
			token, err := ioutil.ReadFile(KubernetesServiceAccountTokenPath)
			if err != nil {
			...
			securityToken, err := newPrincipalToken(string(token))
			if err != nil {
				err = fmt.Errorf("can not create resource principal, Invalid Service Account Token at %s",
					KubernetesServiceAccountTokenPath)
				return nil, err
			}
